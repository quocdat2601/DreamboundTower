...
    public Character GetPlayerCharacter()
    ...
    void SpawnEnemies()
    {
        ...
        for (int i = 0; i < enemyCount; i++)
        {
            ...
            // Add EnemyInfo component for loot system
            EnemyInfo enemyInfo = enemyGO.GetComponent<EnemyInfo>();
            if (enemyInfo == null)
            {
                enemyInfo = enemyGO.AddComponent<EnemyInfo>();
            }
            
            if (finalTemplate != null)
            {
                enemyInfo.enemyKind = finalTemplate.kind;
                enemyInfo.enemyTemplate = finalTemplate;
            }
            else if (overrideUseCustomStats)
            {
                enemyInfo.enemyKind = overrideKind;
                enemyInfo.enemyTemplate = null;
            }
...
    void AutoBindEnemySlotButtons()
    ...
    IEnumerator PlayerAttackRoutine(int enemyIndex)
    {
        ...
        // Note: Only the player regenerates mana - enemies do not regenerate mana automatically
        if (playerCharacter != null)
        {
            ...
            // Apply combined mana regeneration (player only)
...
        persistentPlayer.SetActive(false);
    ...
    private void SpawnIndividualSummonedEnemy(int slotIndex, EnemyTemplateSO template, int? floorOverride = null)
    {
        ...
        enemyInfo.enemyKind = template.kind;
        enemyInfo.enemyTemplate = template;
